...
class Radacon(Module):
    def __init__(self,
        ...
        self.optim_shader = get_shader(file_path + os.sep + 'radacon.comp')

        self.rand_seed_time_buff = self.gpu.manager.tensor(np.zeros((1,)))

        # todo: special buffers that must be provided by the CPU each cycle
        self.cpu_input_buffers = [self.rand_seed_time_buff]
        ...
        self.optim_algorithm = self.gpu.manager.algorithm(
            ...
            spec_consts=np.concatenate(
                (np.asarray([int(min(self.gpu.max_workgroup_invocations, self.w_buf.size()))], dtype=np.uint32).view(
                    np.float32), [0.1, 1e-3, 1e-2]))

        )

        self.basic_sequence = None  # mostly for debugging

    def set_time_seed(self):
        self.rand_seed_time_buff.data()[:] = [time.time()]
