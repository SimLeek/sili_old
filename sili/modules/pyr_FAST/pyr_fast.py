...
import kp
import numpy as np
from displayarray import DirectDisplay, read_updates
import cv2
import os
...
if __name__ == '__main__':
    ...
    while r:
        if not r.frames:
            continue
        if first:
            to_pyr = ToImagePyramid(gpu, r.frames[str(name)][0])
        im_pyr = to_pyr.basic_forward(r.frames[str(name)][0])
        if first:
            conv_pyr = PyrConv(gpu, im_pyr, lambda x: gaussian_center_surround(x, 3))
            first = False
        out_pyr = conv_pyr.basic_forward(im_pyr)
        print(f"time:{conv_pyr.t1 - conv_pyr.t0}, fps:{1. / (conv_pyr.t1 - conv_pyr.t0)}")
        for i, o in enumerate(out_pyr.get()):
            display.imshow(f'output {i}', o)
        #while True:
        display.update()
        if display.window.is_closing:
