...
class ToSpVec(Module):
    def __init__(self,
                 gpu: GPUManager,
                 vec_io: kp.Tensor,
                 spvec_io:kp.Tensor = None,
                 initial_size: int = None,  # for buffers used for multiple things
                 forward=True,
                 backprop=False
                 ):
        """
        ...
        self.initial_size = initial_size if initial_size is not None else self.vec_io.size()

        if spvec_io is None:
            # value -> index value
            self.spvec_io = self.gpu.manager.tensor(np.zeros(self.initial_size*2))
        else:
