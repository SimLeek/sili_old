...
class SparseVectorBuffer(object):
    def __init__(self, gpu: GPUManager, max_nnz:int, dense_len:int):
        ...
        self.buffer = gpu.buffer(np.frombuffer(self.nnz.to_bytes(length=4, signed=False) + sparse_vec.tobytes(), dtype=np.float32))
...
class ToSpVec(Module):
    def __init__(self,
        ...
        """
        self.gpu = gpu
        if not isinstance(vec_io, kp.Tensor):
        ...
        if isinstance(spvec_io, int):
            # value -> index value
            self.spvec_io = SparseVectorBuffer(self.gpu, spvec_io, self.dense_len)
        elif isinstance(spvec_io, SparseVectorBuffer):
            self.spvec_io = spvec_io
        else:
            raise TypeError("spvec_io Buffer must be an int or SparseVectorBuffer. kp.buffer isn't set up yet.")
        ...
        if self.spvec_io.buffer.size() < min_len:
            ...
            raise TypeError(f"sparse vector isn't large enough to store intermediate sparsity calcualtion from full lendth vector. "
                            f"{self.spvec_io.buffer.size()}<{min_len}")
